...
if YAML_SUPPORTED:
    ...
    def load_yaml(file_name):
        ...
        try:
            with open(
                file_name, mode=MODE_READ, encoding=ENCODING
            ) as input_file:
                return yaml.safe_load(input_file)
            #
        except yaml.parser.ParserError as yaml_parser_error:
            raise InvalidFormatError(
                "YAML parsing failed:\n{0}".format(yaml_parser_error.problem)
    ...
    def dump_to_yaml(data, file_name):
        ...
        with open(
            file_name, mode=MODE_WRITE, encoding=ENCODING
        ) as output_file:
...
def compare_data(arguments):
    ...
    for current_file_name in arguments.file_name:
        file_data.append(comparable_form(read_file(current_file_name)))
        labels.append("Data from {0}".format(current_file_name))
        times.append(
...
def translate_files(arguments):
    ...
    if os.path.exists(arguments.output_file_name):
        if not arguments.overwrite:
            print(
                "The output file {0.output_file_name!r} exists already."
        ...
        if arguments.verbose:
            print(
                "Overwriting the existing file {0.output_file_name!r}"
                " as requested".format(arguments)
            )
        #
    #
    write_to_file(
        read_file(arguments.input_file_name), arguments.output_file_name
    )
    return RC_OK
...
def __get_arguments():
    ...
    parser_compare = subparsers.add_parser(
        SUBCOMMAND_COMPARE, help="Compare the data stored in two files."
    )
